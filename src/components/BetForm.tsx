...
interface BetFormProps {
  ...
  cardCount?: number;
  isWinner?: boolean;
  betSummary?: import("../types/game").BetResult[];
}
...
const BetForm: React.FC<BetFormProps> = ({
  ...
  const barColor = useTransform(progress, [1, 0.5, 0], ["#22c55e", "#fde047", "#ef4444"]); // green -> yellow -> red
  ...
  const [bets, setBets] = useState<{ cardIndex: number; amount: number }[]>([]);
  ...
  // Check if card already bet on
  const cardAlreadyBet = (idx: number) => bets.some((b) => b.cardIndex === idx);

  // Modified onPlaceBet to handle multiple bets
  const handlePlaceBet = () => {
    if (disabled || cardIndex === 0 || betAmount <= 0 || cardAlreadyBet(cardIndex)) {
      return;
    }
    setBets((prev) => [...prev, { cardIndex, amount: betAmount }]);
    setCardIndex(0); // reset selection
    setBetAmount(minBet); // reset amount
    // Call parent handler for server sync
    onPlaceBet();
  };
      ...
      <div className='relative flex flex-col gap-4 p-4 sm:p-6 bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl shadow-lg w-full border border-gray-700'>
        ...
        {typeof revealCountdown === "number" && revealCountdown >= 0 && (
          <div className='absolute inset-0 z-30 flex flex-col items-center justify-center bg-black/90 rounded-2xl animate-fade-in backdrop-blur-sm'>
            <span className='text-3xl sm:text-4xl font-bold text-white mb-2 bg-gradient-to-r from-yellow-300 to-yellow-500 bg-clip-text '>
              Revealing in...
            </span>
            <motion.span
            ...
            {betPlaced === false ? (
              <div className='flex flex-col items-center justify-center'>
                <div className='w-24 h-36 sm:w-32 sm:h-48 bg-gradient-to-br from-gray-500 to-gray-700 border-4 border-gray-400 rounded-xl flex items-center justify-center shadow-2xl relative overflow-hidden'>
                  <span className='text-5xl sm:text-7xl font-extrabold text-gray-300 drop-shadow-lg'>{redCard}</span>
                </div>
              </div>
            ) : (
              <>
                ...
                >
                  <div
                    className={`w-24 h-36 sm:w-32 sm:h-48 rounded-xl flex items-center justify-center shadow-2xl relative overflow-hidden ${
                      isWinner
                ...
                <motion.div
                  ...
                  animate={{ opacity: 1 }}
                  transition={{ delay: 1 }}
                  className={`mt-6 text-lg font-medium ${isWinner ? "text-yellow-200" : "text-gray-400"}`}
                >
                  {isWinner ? "Congratulations! Your bet paid off!" : "Better luck next round!"}
                </motion.div>
              </>
            )}
          </div>
        ...
        {showRedCard && redCard !== null && betSummary.length > 0 && (
          <div className='absolute inset-0 z-40 flex flex-col items-center justify-center bg-black/90 rounded-2xl animate-fade-in backdrop-blur-sm'>
            <div className='text-2xl font-bold mb-4 text-yellow-200'>Your Bet Results</div>
            <ul className='mb-4'>
              {betSummary.map((b, i) => (
                <li key={i} className={`text-lg font-mono mb-1 ${b.won ? "text-green-400" : "text-red-400"}`}>
                  Card {b.cardIndex}: {b.won ? `Won! +${b.payout}` : `Lost (-${b.amount})`}
                </li>
              ))}
            </ul>
            {/* Optionally, show total won/lost */}
            <div className='text-lg font-semibold text-white'>
              Total: {betSummary.reduce((sum, b) => sum + (b.payout - (b.won ? 0 : b.amount)), 0)}
            </div>
          </div>
        )}
        ...
        <div
          className={`w-full p-4 rounded-xl ${
            isSelectionRequired ? "bg-yellow-500/10 ring-2 ring-yellow-400 animate-pulse" : "bg-gray-700/50"
          }`}
        >
          <CardSelector
            ...
            disabled={disabled}
            // Disable cards already bet on
            disabledIndices={bets.map((b) => b.cardIndex)}
          />
        </div>
        {/* Bets summary */}
        {bets.length > 0 && (
          <div className='mt-2 mb-2 p-2 bg-gray-800 rounded-lg text-white text-sm'>
            <div className='font-bold mb-1'>Your Bets This Round:</div>
            <ul className='list-disc list-inside'>
              {bets.map((b, i) => (
                <li key={i}>
                  Card {b.cardIndex}: <span className='font-mono'>{b.amount}x</span>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className='flex flex-col gap-4 mt-2'>
          <div className='flex flex-col items-center w-full'>
            <label className='text-white/80 text-sm mb-2 uppercase tracking-wider'>Bet Amount</label>
          ...
          <button
            ...
            className={`w-full py-3 rounded-xl shadow-lg transition-all text-base sm:text-lg font-bold mt-2
              ${
                disabled || cardIndex === 0 || cardAlreadyBet(cardIndex)
                  ? "bg-gray-600 text-gray-400 cursor-not-allowed"
                  : "bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white cursor-pointer"
              }`}
            disabled={disabled || cardIndex === 0 || cardAlreadyBet(cardIndex)}
            title={cardAlreadyBet(cardIndex) ? "Already bet on this card" : "Place your bet"}
          >
            {cardAlreadyBet(cardIndex) ? "Already Bet" : "Place Bet"}
          </button>
          ...
          <div className='flex flex-col gap-2 mt-2'>
            <div className='flex items-center justify-between text-sm text-white/80'>
              ...
              <motion.span
                ...
                animate={{
                  scale: pulse ? [1, 1.25, 1] : timeLeft <= 3 ? [1, 1.15, 1] : 1,
                  color: timeLeft <= 3 ? ["#fff", "#ef4444", "#fff"] : undefined,
                  textShadow: pulse ? "0 0 12px #fde047, 0 0 24px #fde047" : undefined,
                }}
                transition={{
